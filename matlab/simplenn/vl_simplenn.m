function [res, res_depth] = vl_simplenn(net, x, dzdy, res, varargin)
...
if nargin <= 3 || isempty(res)
  ...
  res_depth = struct(...
    ...
    'dzdw', cell(1,pp+1), ...
    'aux', cell(1,pp+1), ...
    'time', num2cell(zeros(1,pp+1)), ...
    'backwardTime', num2cell(zeros(1,pp+1))) ;
end
res(1).x = x(:,:,2:4,:) ;
res_depth(1).x = x(:,:,1,:) ;
    ...
    case 'conv'
      if isfield(l, 'weights')
        res(i+1).x = vl_nnconv(res(i).x, l.weights{1}, l.weights{2}, ...
                               ...
                               cudnn{:}) ;   
        if(i<=5)
        res_depth(i+1).x = vl_nnconv(res_depth(i).x, l.weights_depth{1}, l.weights_depth{2}, ...
                               'pad', l.pad_depth, 'stride', l.stride_depth, ...
                              cudnn{:}) ; 
        end
...
%              x=gather(x);
            temp=zeros(size(res(i+1).x, 1),size(res(i+1).x, 2),2);
            temp(:,:,1)=res(i+1).x;
            temp(:,:,2)=res_depth(i+1).x;
            res(i+1).x=single(temp);
%              res(i+1).x=gpuArray(res(i+1).x);
%              x=gpuArray(x);
        end
    ...
    case 'relu'
      ...
      if(i<=5)
          res_depth(i+1).x = vl_nnrelu(res_depth(i).x) ;
      end
    case 'sigmoid'
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    switch l.type
      case 'conv'
        if ~opts.accumulate
          if isfield(l, 'weights')
            ...
            [res_depth(i).dzdx, res_depth(i).dzdw{1}, res_depth(i).dzdw{2}] = ...
                vl_nnconv(res_depth(i).x, l.weights_depth{1}, l.weights_depth{2}, ...
                          res(i+1).dzdx(:,:,2,:), ...
                          'pad', l.pad_depth, 'stride', l.stride_depth, ...
                          cudnn{:}) ;
              else
      ...
      case 'relu'
        ...
        if(i<=5)
              if ~isempty(res(i).x)
                    res_depth(i).dzdx = vl_nnrelu(res_depth(i).x, res_depth(i+1).dzdx) ;
              else
                    res_depth(i).dzdx = vl_nnrelu(res_depth(i+1).x, res_depth(i+1).dzdx) ;
              end
        end
      case 'sigmoid'
